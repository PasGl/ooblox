...
{
	this.mesh = new THREE.Mesh( new THREE.BoxGeometry(1, 1, 0.5), [ new THREE.MeshStandardMaterial({color:0xffffff}) ,   new THREE.MeshStandardMaterial({color:0x000000}) ]  );
	...
	{
		if (conf.lights) {
			mesh.material[0] = new THREE.MeshLambertMaterial({
			...
			mesh.material[0] = new THREE.MeshBasicMaterial({
				map: (new THREE.TextureLoader().load( "images/textures/" + conf.textureFilename )),
				transparent: Boolean(conf.transparent),
				opacity: conf.opacity
			});
	...
	{
		mesh.material[0].map = new THREE.TextureLoader().load( "images/textures/" + conf.textureFilename );
	...
	var fillDatGUI = function (targetScene,mesh)
	{
		...
		transSwitch.onChange(function(){mesh.material[0].transparent=Boolean(conf.transparent);refreshURL(targetScene);});
		var opacitySlider = propFolder.add(conf,'opacity',0.0,1.0).name("Opacity").step(0.0001);
		opacitySlider.onChange(function(){mesh.material[0].opacity=conf.opacity;refreshURL(targetScene);});
